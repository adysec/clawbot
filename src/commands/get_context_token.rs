...
pub async fn run(
    ...
    if let Ok(Some(session)) = db::load_session(&user_id) {
        if !session.context_token.is_empty() {
            println!("{}", session.context_token);
            eprintln!("restored cached context token from database");
            return Ok(());
        }
    }
    ...
    eprintln!("waiting for {} to send a message; press Ctrl+C to stop", user_id);
            ...
            Ok(resp) => {
                ...
                for message in resp.messages() {
                    if let Some(context_token) = extract_context_token(&user_id, message) {
                        ...
                        eprintln!("context token saved to database");
                        return Ok(());
                    }
                }
            }
