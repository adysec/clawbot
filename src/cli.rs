...
pub enum Command {
    ...
    Send(SendArgs),
    /// Interactive chat mode (see messages and reply in real time).
    Chat(ChatArgs),
    /// Start an HTTP webhook server for sending messages via API.
    Webhook(WebhookArgs),
    /// Start the web management platform (multi-user, QR bind, webhook).
    Serve(ServeArgs),
}
...
pub struct WebhookArgs {
    #[arg(
        long,
        default_value = "11888",
        help = "Port to listen on (default: 11888)."
    )]
    pub port: u16,
    #[arg(
        long,
