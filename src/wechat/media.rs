...
use cipher::{BlockModeEncrypt as _, KeyInit};
use ecb;
...
pub async fn upload_media(
    ...
    // Read file on blocking thread pool
    ...
    // Encrypt on blocking thread pool (CPU-bound)
    let encrypted = tokio::task::spawn_blocking(move || encrypt_aes_ecb(&key, &data))
        .await
